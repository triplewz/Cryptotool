...
// NewSignature creates a new sidemix signature (Schnorr-type signature)
...
func (sig *Signature) Ver(Disclosure []byte, ipk *IssuerPublicKey, gpk *GroupPublicKey, msg []byte, attributeValues []*FP256BN.BIG) error {
	...
	ProofSalpha := FP256BN.FromBytes(sig.GetProofSalpha())
	ProofSbeta := FP256BN.FromBytes(sig.GetProofSbeta())
	ProofSx := FP256BN.FromBytes(sig.GetProofSx())
	ProofSdelta1 := FP256BN.FromBytes(sig.GetProofSdelta1())
	ProofSdelta2 := FP256BN.FromBytes(sig.GetProofSdelta2())
	U := EcpFromProto(gpk.U)
	V := EcpFromProto(gpk.V)
	H := EcpFromProto(gpk.H)
	s1 := Modadd(ProofSalpha,ProofSbeta,GroupOrder)
	s2 := Modadd(ProofSdelta1,ProofSdelta2,GroupOrder)
	...
	R1 := FP256BN.G1mul(U,ProofSalpha)
	R1.Sub(T1.Mul(ProofC))
	R2 := FP256BN.G1mul(V,ProofSbeta)
	R2.Sub(T2.Mul(ProofC))
	Gsx := GenG2.Mul(ProofSx)
	e1 := FP256BN.Ate(Gsx,T3)
	Hs1 := H.Mul(s1)
	e2 := FP256BN.Ate(W,Hs1)
	e2.Inverse()
	e1.Mul(e2)
	Hs2 := H.Mul(s2)
	e4 := FP256BN.Ate(GenG2,Hs2)
	e4.Inverse()
	e1.Mul(e4)
	Wc := W.Mul(ProofC)
	e6 := FP256BN.Ate(Wc,T3)
	Gc := GenG2.Mul(ProofC)
	e7 := FP256BN.Ate(Gc,GenG1)
	e7.Inverse()
	e7.Mul(e6)
	e7.Mul(e1)
	R3 := FP256BN.Fexp(e7)
	R4 := FP256BN.G1mul(T1,ProofSx)
	R4.Sub(U.Mul(ProofSdelta1))
	R5 := FP256BN.G1mul(T2,ProofSx)
	R5.Sub(V.Mul(ProofSdelta2))
